...
public class WarnSystem {
	...
	public static void Warn(Player sender, String PlayerString){
		...
		if(player == null){ sender.sendMessage(Holder.title + "§8§lThe player: " + PlayerString + " wasn't online!"); return; }
		...
		if(!WarnList.containsKey(player)){
			...
			player.damage(2);
            player.sendMessage(Holder.title + "§4§lYou have been warned by: " + sender.getDisplayName() + ".");
			...
			player.damage(10);
            String StartMessage = Holder.title + "§l§4You have been WarnBanned by " + sender.getDisplayName() + "!";
			String MiddleMessage = "§b" + Holder.WarnKickMessage;
