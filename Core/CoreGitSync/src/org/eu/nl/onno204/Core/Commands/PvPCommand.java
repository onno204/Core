...
public class PvPCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {  
		...
		if(args[0].equalsIgnoreCase("on")){ 
			if(option){
				sender.sendMessage(Holder.title + "Pvp was already turned on.");
				return true;
			}
			...
			if(!option){
				sender.sendMessage(Holder.title + "Pvp was already turned off.");
				return true;
			}
			...
			sender.sendMessage(Holder.title + "The currently pvp enabled: " + Holder.PvPEnabled);
